...
// It returns nil for a context that carries no halt signal (because a receive on a nil channel blocks forever, selecting on the result is always safe)
func HaltingFromContext(ctx context.Context) <-chan struct{} {
	v, _ := ctx.Value(haltingCtxKey{}).(chan struct{})
	return v
}
...
func WithHalting(ctx context.Context, haltCh chan struct{}) context.Context {
	return context.WithValue(ctx, haltingCtxKey{}, haltCh)
}
