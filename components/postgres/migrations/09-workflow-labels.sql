...
CREATE INDEX %pactor_state_wf_status_idx ON %sactor_state (actor_type, (workflow_labels->>'status'), actor_id)
    WHERE workflow_labels IS NOT NULL;
CREATE INDEX %pactor_state_wf_version_idx ON %sactor_state (actor_type, (workflow_labels->>'version'), actor_id)
    WHERE workflow_labels IS NOT NULL;
CREATE INDEX %pactor_state_wf_parent_idx ON %sactor_state (actor_type, (workflow_labels->>'parent'), actor_id)
    WHERE workflow_labels IS NOT NULL;
