...
func TestSQLiteConnectionString_Parse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:          ":memory:",
			expectedResult: "file::memory:?_pragma=busy_timeout%282500%29&_pragma=foreign_keys%281%29&_pragma=journal_mode%28MEMORY%29&_txlock=immediate",
			shouldError:    false,
			description:    "should set journal_mode=MEMORY for in-memory DB",
		},
		{
			...
			expectedResult: "file::memory:?_pragma=busy_timeout%282500%29&_pragma=foreign_keys%281%29&_pragma=journal_mode%28MEMORY%29&_txlock=immediate",
			shouldError:    false,
			description:    "should set journal_mode=MEMORY for file in-memory DB",
		},
		{
			name:           "mode=memory",
			input:          "file:data?mode=memory",
			expectedResult: "file:data?mode=memory&_pragma=busy_timeout%282500%29&_pragma=foreign_keys%281%29&_pragma=journal_mode%28MEMORY%29&_txlock=immediate",
			shouldError:    false,
			description:    "should set journal_mode=MEMORY for file in-memory DB",
		},
		{
