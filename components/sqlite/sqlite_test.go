...
func (s *SQLiteProvider) Seed(ctx context.Context, spec comptesting.Spec) error {
	_, tErr := transactions.ExecuteInSqlTransaction(ctx, s.log, s.db, s.timeout, "IMMEDIATE", func(ctx context.Context, tx *sql.Conn) (z struct{}, err error) {
		now := s.clock.Now()
...
func (s *SQLiteProvider) GetAllHosts(ctx context.Context) (comptesting.Spec, error) {
	return transactions.ExecuteInSqlTransaction(ctx, s.log, s.db, s.timeout, "IMMEDIATE", func(ctx context.Context, tx *sql.Conn) (res comptesting.Spec, err error) {
		// Load all hosts
