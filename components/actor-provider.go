...
type RegisterHostReq struct {
	...
	// JoinToken is the jti from the JWT bootstrap token; when non-empty, the provider records it to prevent replay
	// Empty when the token carried no jti or no expiry, and for non-JWT auth paths; must not be stored when empty
	JoinToken string
