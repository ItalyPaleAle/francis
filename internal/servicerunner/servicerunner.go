	...
	for _, service := range r.services {
		go func(service Service) {
			...
			if rErr != nil && !errors.Is(rErr, context.Canceled) {
				errCh <- rErr
				// Cancel the context to stop the other services, with the error
				// If the context has been canceled already, this is a no-op
				cancel(rErr)
				return
			}

			// Cancel the context to stop other services
			cancel(nil)
			errCh <- nil
		}(service)
	}
