...
// Both endpoints of the same session derive identical bytes, while a meddler-in-the-middle (MitM) that terminates TLS sees a different value, so a proof bound to this value cannot be relayed onto another connection
