...
func TestHostHalt(t *testing.T) {
	...
	t.Run("deactivation timeout", func(t *testing.T) {
		...
		shortTimeoutHost := &Manager{
			removeActor: timeoutProvider.RemoveActor,
			Actors:      haxmap.New[string, *ActiveActor](8),
			log:         log,
			clock:       clock,
			ActorsConfig: map[string]components.ActorHostType{
		...
		shortTimeoutHost.IdleProcessor = eventqueue.NewProcessor(eventqueue.Options[string, *ActiveActor]{
...
func TestHostHaltAll(t *testing.T) {
	...
	t.Run("halt all with some actor failures", func(t *testing.T) {
		...
		err := host.HaltAll()
		require.Error(t, err)
		require.ErrorContains(t, err, "error halting actors")
