...
| Registry | `francis.builtin.workflow.<name>.registry` | the cluster-wide singleton | Records fingerprints and generations, authorizes starts, and serializes their post-journal confirmation with version resets. |
...
- **The orchestrator never calls another actor on behalf of user logic.** A synchronous invocation couples this instance's turn lock to another actor's availability and queue depth. Talking to another actor is a step. The engine makes one framework-owned exception: registry authorization for new starts, post-journal confirmation, and legacy identity migration. Established deliveries check the persisted graph identity locally.
