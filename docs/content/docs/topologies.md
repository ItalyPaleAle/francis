...
Francis can run in two **topologies**. They differ in where the data store and coordination live — not in how you write actors. Your actor code (factories, `Invoke`, `Alarm`, state, alarms) is **identical** in both; only the host setup changes.
...
| Extra process to run | None | The `runtime` control plane |
| Worker process | Self-contained | Stateless; connects to the runtime |
| Typical use | Single-node apps, embedded use, development, small clusters | Larger clusters, separation of control plane from workers |
...
- You're running a single node, or a small cluster that can share a database.
...
- You're running a larger cluster and prefer a dedicated coordination tier.
- You want to scale workers independently of where state lives.

Because the runtime persists state to its database, actor state survives a worker restart **and** a runtime restart.
...
|----------|--------------|-----------------------------------|-------|
| SQLite | `WithSQLiteProvider` | `sqlite` | File-based; great for single-node and development. Must not live on a networked filesystem (NFS/SMB). |
| PostgreSQL | `WithPostgresProvider` | `postgres` | For multi-node clusters that share one database. |
| In-memory | `WithStandaloneMemoryProvider` | `memory` | Non-durable; for tests and ephemeral setups. State is lost on restart. |
